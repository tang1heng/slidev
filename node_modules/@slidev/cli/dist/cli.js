...
var _chunkT7KA74KVjs = require('./chunk-T7KA74KV.js');
...
var import_fast_deep_equal = _chunkN6L4BUFDjs.__toESM.call(void 0, _chunk2IBHRHAWjs.require_fast_deep_equal.call(void 0, ));
...
var cli = _yargs2.default.scriptName("slidev").usage("$0 [args]").version(_chunkT7KA74KVjs.version).strict().showHelpOnFail(false).alias("h", "help").alias("v", "version");
cli.command(
  "* [entry]",
  ...
  async ({ entry, theme, port: userPort, open, log, remote, force, inspect }) => {
    ...
    async function initServer() {
      if (server)
        await server.close();
      const options = await _chunkT7KA74KVjs.resolveOptions.call(void 0, { entry, remote, theme, inspect }, "dev");
      port = userPort || await findFreePort(3030);
      server = await _chunkT7KA74KVjs.createServer.call(void 0, 
        options,
        {
          server: {
            port,
        ...
        },
        {
          onDataReload(newData, data) {
            if (!theme && _chunkT7KA74KVjs.resolveThemeName.call(void 0, newData.config.theme) !== _chunkT7KA74KVjs.resolveThemeName.call(void 0, data.config.theme)) {
...
cli.command(
  ...
  async ({ entry, theme, watch, base, download, out, inspect }) => {
    ...
    const options = await _chunkT7KA74KVjs.resolveOptions.call(void 0, { entry, theme, inspect }, "build");
...
cli.command(
  ...
  async ({ entry }) => {
    const data = await _chunkT7KA74KVjs.parser.load(entry);
    _chunkT7KA74KVjs.parser.prettify(data);
    await _chunkT7KA74KVjs.parser.save(data);
  }
);
cli.command(
  "theme [subcommand]",
  ...
  (command) => {
    return command.command(
      ...
      async ({ entry, dir, theme: themeInput }) => {
        const data = await _chunkT7KA74KVjs.parser.load(entry);
        const theme = _chunkT7KA74KVjs.resolveThemeName.call(void 0, themeInput || data.config.theme);
        if (theme === "none") {
          console.error('Can not eject theme "none"');
          process.exit(1);
        }
        if (_chunkT7KA74KVjs.isPath.call(void 0, theme)) {
          console.error("Theme is already ejected");
          process.exit(1);
        }
        const roots = _chunkT7KA74KVjs.getThemeRoots.call(void 0, theme, entry);
        if (!roots.length) {
...
cli.command(
  ...
  async ({
    ...
    const options = await _chunkT7KA74KVjs.resolveOptions.call(void 0, { entry, theme }, "build");
    output = output || options.data.config.exportFilename || `${_path2.default.basename(entry, ".md")}-export`;
    const server = await _chunkT7KA74KVjs.createServer.call(void 0, 
      options,
      {
        server: { port },
        clearScreen: false
    ...
    printInfo(options);
    _chunkT7KA74KVjs.parser.filterDisabled(options.data);
...
cli.command(
  ...
  async ({
    ...
    const port = await findFreePort(12445);
    const options = await _chunkT7KA74KVjs.resolveOptions.call(void 0, { entry }, "build");
    if (!output)
...
function printInfo(options, port, remote) {
  ...
  console.log(`${_kolorist.bold.call(void 0, "  Slidev")}  ${_kolorist.blue.call(void 0, `v${_chunkT7KA74KVjs.version}`)} ${_isinstalledglobally2.default ? _kolorist.yellow.call(void 0, "(global)") : ""}`);
  console.log();
